...
function Screener({ onAnalyze }) {
  ...
  async function search() {
    ...
    try {
      const data = await searchPubmed(query.trim(), 15)
      setResults(data)
    } catch(e) {
      setError(e.message)
    } finally {
  ...
  async function analyzeSelected() {
    const toAnalyze = results.papers.filter(p => selected.has(p.pmid))
    ...
    for (let i = 0; i < toAnalyze.length; i++) {
      ...
      try {
        ...
        if (paper.doi) {
          ...
          result = await analyzeByDoi(`pmid:${paper.pmid}`)
        }
        resultsList.push(result)
      } catch(e) {
        console.error(`Error analizando ${paper.pmid}:`, e.message)
      }
    }
    setAnalyzing(false)
    setProgress('')
    if (resultsList.length > 0) onAnalyze(resultsList)
  }

  const inputStyle = {
    width: '100%', padding: '.75rem 1rem', boxSizing: 'border-box',
    border: '1.5px solid #e2e8f0', borderRadius: 10, fontSize: '.9rem',
    fontFamily: "'Georgia', serif", outline: 'none', color: '#1e293b', background: C.white,
  }
      ...
      <div style={{ background: C.white, borderRadius: 16, padding: '1.25rem', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            placeholder="ej: total knee arthroplasty implant selection 2023"
          ...
          <button
            ...
            style={{
              padding: '.75rem 1.25rem', border: 'none', borderRadius: 10,
        ...
        {error && (
          <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '.6rem 1rem', marginTop: '1rem', color: '#dc2626', fontSize: '.85rem' }}>
            ⚠️ {error}
          </div>
        )}
        <p style={{ margin: '8px 0 0', color: C.gray, fontSize: '.75rem' }}>
          Resultados de PubMed · Selecciona los que quieres analizar con IA
        </p>
      </div>
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.75rem' }}>
            <p style={{ margin: 0, color: C.gray, fontSize: '.85rem' }}>
              <strong style={{ color: C.navy }}>{results.total}</strong> papers encontrados
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.75rem' }}>
            {results.papers.map(paper => {
              const isSelected = selected.has(paper.pmid)
              return (
                <div
                  key={paper.pmid}
                  onClick={() => toggleSelect(paper.pmid)}
                  style={{
                    background: isSelected ? '#eff6ff' : C.white,
                    border: `2px solid ${isSelected ? C.blue : '#e2e8f0'}`,
                    borderRadius: 14, padding: '1rem',
                          cursor: 'pointer', transition: 'all .15s',
                    boxShadow: isSelected ? `0 0 0 3px rgba(26,107,181,0.1)` : '0 2px 8px rgba(0,0,0,0.04)',
                  }}
                >
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    <div style={{
                      width: 20, height: 20, borderRadius: 6, flexShrink: 0, marginTop: 2,
                      border: `2px solid ${isSelected ? C.blue : '#cbd5e1'}`,
                      background: isSelected ? C.blue : 'transparent',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                      {isSelected && <span style={{ color: '#fff', fontSize: '.7rem', fontWeight: 800 }}>✓</span>}
                    </div>
                    <div style={{ flex: 1 }}>
                      <p style={{ margin: '0 0 4px', fontWeight: 700, color: C.navy, fontSize: '.9rem', lineHeight: 1.4 }}>
                        {paper.title}
                      </p>
                      <p style={{ margin: '0 0 8px', color: C.gray, fontSize: '.78rem' }}>
                        {paper.authors} · <em>{paper.journal}</em> · {paper.year}
                      </p>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {paper.doi && (
                          <span style={{ background: '#dcfce7', color: '#16a34a', borderRadius: 6, padding: '2px 8px', fontSize: '.7rem', fontWeight: 700 }}>
                            ✓ DOI disponible
                          </span>
                        )}
                        <span style={{ background: '#f1f5f9', color: C.gray, borderRadius: 6, padding: '2px 8px', fontSize: '.7rem' }}>
                          PMID: {paper.pmid}
                        </span>
                      </div>
                    </div>
...
function InputPanel({ onResult, initialMode = 'doi' }) {
  ...
  const tabStyle = (active) => ({
    ...
    color: active ? '#fff' : C.gray, transition: 'all .2s',
  })
    ...
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      {mode === 'doi' ? (
        <div>
          ...
          <input
            value={doi} onChange={e => { setDoi(e.target.value); setError('') }}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Pega aquí el abstract o texto completo..." rows={8}
            style={{ ...inputStyle, resize: 'vertical', lineHeight: 1.7 }}
            onFocus={e => e.target.style.borderColor = C.blue2}
            onBlur={e  => e.target.style.borderColor = '#e2e8f0'}
          />
          <input
            value={textDoi} onChange={e => setTextDoi(e.target.value)}
            placeholder="DOI (opcional)" style={{ ...inputStyle, marginTop: 8, fontSize: '.82rem' }}
          />
        </div>
      )}

      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '.75rem 1rem', marginTop: '1rem', color: '#dc2626', fontSize: '.85rem' }}>
          ⚠️ {error}
        </div>
      )}

      <button onClick={submit} disabled={loading || !canSubmit} style={{
        marginTop: '1.5rem', width: '100%', padding: '1rem', border: 'none', borderRadius: 12,
        background: loading || !canSubmit ? '#94a3b8' : `linear-gradient(135deg, ${C.navy}, ${C.blue})`,
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '1rem', overflowX: 'auto', paddingBottom: 4 }}>
          {results.map((r, i) => (
            <button key={i} onClick={() => setIdx(i)} style={{
              padding: '.4rem .9rem', border: 'none', borderRadius: 8, cursor: 'pointer',
              background: i === idx ? C.navy : '#e2e8f0',
              color: i === idx ? '#fff' : C.gray,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px,1fr))', gap: '1rem', marginBottom: '1rem' }}>
        <Section icon="🏥" title="Aplicabilidad Clínica">
          <p style={{ margin: 0, color: '#1e293b', lineHeight: 1.7, fontSize: '.9rem' }}>{result.aplicabilidad_clinica}</p>
        </Section>
        <Section icon="🇨🇱" title="Contexto Chile / LATAM">
          <p style={{ margin: 0, color: '#1e293b', lineHeight: 1.7, fontSize: '.9rem' }}>{result.aplicabilidad_chile}</p>
        </Section>
      </div>
  ...
  const navTab = (key, label) => (
    <button
      ...
      style={{
        ...
        background: tab === key ? C.navy : 'transparent',
