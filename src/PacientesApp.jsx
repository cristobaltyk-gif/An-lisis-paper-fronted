...
export default function PacientesApp() {
  ...
  async function runQuery(text) {
    ...
    try {
      await pacientesQuery(text, {
        ...
        onText: (chunk) => { update({ step: 3 }); setConsultas(prev => prev.map(c => c.id === id ? { ...c, response: c.response + chunk } : c)) },
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #f0faf5 0%, #e8f5ee 50%, #f5faf7 100%)', fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      <style>{`* { box-sizing: border-box; } .pill:hover { background: #2D6A4F !important; color: white !important; } .pill { transition: all .2s ease !important; cursor: pointer; } input:focus { outline: none; }`}</style>
